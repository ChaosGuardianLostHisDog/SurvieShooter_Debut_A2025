...
public class TirScript_RayCast : MonoBehaviour
{
    ...
    */
    ...
    void ActiveTir()
    {
        ...
        RaycastHit infoCollisions;

        if(Physics.Raycast(boutDuFusil.transform.position, boutDuFusil.transform.forward, out infoCollisions, 50f))
        {
            GameObject impact = Instantiate(impactTir, infoCollisions.point, Quaternion.identity);
